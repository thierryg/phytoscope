#  ==========================================================================
#  PhytoScope — attribution — packaging/templates/linux/frontend.sh
#
#  Version   : 1.6.0
#  Date      : 2026-09-23
...
ui_question() {
    ...
    case "$INTERFACE" in
        ...
        tui)
            ...
            then echo "oui"; else echo "non"; fi ;;
        *)
            #  In text mode the default letter is the capital one.
            #
            #  Both the prompt AND the accepted letters come from the
            #  catalogue. The first version hard-coded "[O/n]" and accepted
            #  only "[oOyY]": a Turkish user typed "e" for evet, a Polish one
            #  "t" for tak, and the installer understood no. English "y" stays
            #  accepted everywhere, because terminal habits expect it.
            if [ "$defaut" = "oui" ]
            then invite="${T_INVITE_OUI_DEFAUT:-[Y/n]}"
            else invite="${T_INVITE_NON_DEFAUT:-[y/N]}"
            fi
            printf "  %s %s " "$texte" "$invite" >&2
            read -r r </dev/tty 2>/dev/null || r=""
            #  No answer: take the default directly. Going through the
            #  letters would be pointless — the default is already known.
            if [ -z "$r" ]; then echo "$defaut"; else
                _oui=non
    ...
    case "$INTERFACE" in
        graphique)
            case "$OUTIL_GUI" in
                zenity)
                    if zenity --text-info \
                        --title="$TITRE — ${T_ENTETE_EDITEUR:-licence}" \
                        --filename="$fichier" --width=680 --height=480 \
                        --checkbox="${T_LICENCE_CASE:-I have read and accept the MIT licence}" \
                        2>/dev/null; then echo "accepte"; else echo "refuse"; fi ;;
                kdialog)
                    kdialog --title "$TITRE — ${T_ENTETE_EDITEUR:-licence}" \
                        --textbox "$fichier" \
                        680 480 2>/dev/null || true
                    if kdialog --title "$TITRE" --yesno \
                        "${T_LICENCE_QUESTION:-Do you accept the MIT licence?}" \
                        2>/dev/null
                    then echo "accepte"; else echo "refuse"; fi ;;
                *) echo "accepte" ;;
            esac ;;
        tui)
            $OUTIL_TUI --title "${T_LICENCE_TITRE:-MIT licence}" \
                --textbox "$fichier" 22 76 \
                2>/dev/null || true
            if $OUTIL_TUI --title "${T_LICENCE_TITRE:-MIT licence}" --yesno \
                "${T_LICENCE_QUESTION:-Do you accept the MIT licence?}" \
                8 60 3>&1 1>&2 2>&3
            then echo "accepte"; else echo "refuse"; fi ;;
        *)
